...
def convert_theats_to_buckets(times, thetas):
    #Create a holding array
    #Create a new time holding array
    #While same time add to it
    #If new time create new bucket

    output = []
    for ii in range(len(times)):
        if output = [] or times[ii] != output[-1][-1]:
            output.append([])
        output[-1].append(theatas[ii])
    return output
